...
@Controller public class ClipController {
	@Autowired private ClipService clipService;
	@Autowired private UserService userService;

	private String urlError =
		"Source URL is not valid. Note: You should add http:// or https://";
	...
	String clipListPage(@PathVariable(required = false) String sub, Model
		...
		if (principal != null) {
			...
			url = String.format("/clips/%s/addEdit", HandleCookie.readCookie(
	...
	@GetMapping("/clips/{sub}/addEdit") public
	String showAddEditClipPageForCreate(@PathVariable(required =
		true) String sub, Clip clip, Model model, HttpServletRequest request) {
		model.addAttribute("user", userService.getUserBySub(
			HandleCookie.readCookie(request, HandleCookie.COOKIE_NAME)));
	...
	@PostMapping("/clips/{sub}/addEdit/add") public String addClip(
		@PathVariable(required = true) String sub, @Valid Clip clip,
	...
	@GetMapping("/clips/{sub}/addEdit/{id}") public
	String showAddEditClipPageForEdit(@PathVariable(required = true) String
	...
	@RequestMapping("/clips/{sub}/addEdit/edit/{id}") public String editClip(
		@PathVariable(required = true) String sub, @PathVariable Long id,
		@Valid Clip clip, BindingResult result, Model model, HttpServletRequest
		request)
	throws Exception {
		model.addAttribute("user", userService.getUserBySub(
			HandleCookie.readCookie(request, HandleCookie.COOKIE_NAME)));
	...
	@RequestMapping("/clips/{sub}/delete/{id}") public String deleteClip(
		@PathVariable(required = true) String sub, @PathVariable Long id, Model
		model, HttpServletRequest request, @AuthenticationPrincipal OidcUser
		principal) {
		model.addAttribute("user", userService.getUserBySub(
			HandleCookie.readCookie(request, HandleCookie.COOKIE_NAME)));
